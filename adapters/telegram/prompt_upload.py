...
Used by the Telegram inbound path to recognize captions like ``/persona
prompt`` or ``/set global_prompt`` so an attached .txt file's contents
become the new prompt instead of regular chat content.
"""
...
from application.use_cases.persona import apply_global_prompt_from_text, apply_persona_prompt_from_text
