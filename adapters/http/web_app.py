...
def build_public_app() -> FastAPI:
    ...
    try:
        ...
        from adapters.http.openapi_tools.terminal_routes import build_terminal_app
        app.mount("/tools/terminal", build_terminal_app())
        app.mount("/tools/search", build_search_app())
        logger.info("adapters.http.web_app: mounted /tools/terminal and /tools/search as separate sub-apps")
    except Exception:
        logger.exception("adapters.http.web_app: failed to mount /tools — public server still serves health")
