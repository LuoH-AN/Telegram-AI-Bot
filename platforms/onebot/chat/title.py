"""Auto title generation for new OneBot sessions."""
...
async def generate_and_set_title(user_id: int, session_id: int, user_message: str, ai_response: str) -> None:
    try:
        ...
        if title:
            ...
            logger.info("%s auto-generated session title: %s", onebot_ctx(user_id), title)
    except Exception as exc:
        logger.warning("%s failed to auto-generate title: %s", onebot_ctx(user_id), exc)
