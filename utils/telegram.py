...
async def send_message_draft_safe(
    ...
    *,
    ...
    """Send/update a Telegram draft stream message with HTML fallback."""
    if len(text) > MAX_MESSAGE_LENGTH:
        text = text[: MAX_MESSAGE_LENGTH - 3] + "..."
    ...
    for _attempt in range(2):
        try:
            ...
            logger.warning(f"Draft HTML parse failed, using plain text: {e}")
            try:
                return bool(
                    await bot.send_message_draft(
                        chat_id=chat_id,
                ...
                logger.warning(f"Failed to send message draft: {e2}")
                return False
        except Exception as e:
            logger.warning(f"Failed to send message draft: {e}")
            return False
    return False
