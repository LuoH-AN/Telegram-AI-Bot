...
def build_provider_no_saved_message(prefix: str) -> str:
    return (
        ...
        "**Usage:**\n"
        f"• `{prefix}set provider save <name>` - save current API infrastructure.config\n"
        f"• `{prefix}set provider load <name>` - load saved infrastructure.config\n"
        f"• `{prefix}set provider delete <name>` - delete saved infrastructure.config"
    )
