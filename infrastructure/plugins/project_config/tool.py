"""AI tool for repository infrastructure.config and infrastructure.database inspection and edits."""
...
def _get_db(user_id: int):
    ...
    from infrastructure.database.loaders import (
...
class ProjectConfigTool(BaseTool):
        ...
        return {
            ...
            "properties": {
                ...
                "target": {
                    ...
                    "description": "Database table/resource to access (source=infrastructure.database)",
                },
                "persona": {"type": "string", "description": "Persona name for session/conversation queries (source=infrastructure.database)"},
                "session_id": {"type": "integer", "description": "Session ID for conversation queries (source=infrastructure.database)"},
                "skill_name": {"type": "string", "description": "Skill name for skill_state queries (source=infrastructure.database)"},
            },
            "required": ["action"],
        }
    ...
    def execute(self, user_id: int, tool_name: str, arguments: dict) -> str:
        ...
        if source == "infrastructure.database":
    ...
    def _execute_db(self, user_id: int, action: str, arguments: dict) -> str:
        ...
        if action not in _VALID_DB_ACTIONS:
            return f"Error: invalid action '{action}' for infrastructure.database. Use: {', '.join(sorted(_VALID_DB_ACTIONS))}"

        if not target:
            return f"Error: target required for infrastructure.database operations. Options: {', '.join(sorted(_VALID_DB_TARGETS))}"
        ...
        try:
            ...
            return f"Error: infrastructure.database operation failed - {exc}"
    ...
    def _upsert_persona(self, user_id: int, name: str, system_prompt: str) -> None:
        ...
        try:
            ...
            conn.close()
        from infrastructure.plugins.project_config.sync.db import refresh_cache_from_db
...
def get_dict_cursor(conn):
    from infrastructure.database.db import get_dict_cursor as _gdc
    return _gdc(conn)
