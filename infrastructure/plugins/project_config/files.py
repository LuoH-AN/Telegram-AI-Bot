...
from infrastructure.plugins.terminal.state import REPO_ROOT
...
def discover_env_keys() -> list[str]:
    files = list((REPO_ROOT / "infrastructure.config").rglob("*.py")) + list((REPO_ROOT / "entrypoints.launcher").rglob("*.py"))
    files += list((REPO_ROOT / "domain.services").rglob("*.py")) + list((REPO_ROOT / "platforms").rglob("*.py")) + [REPO_ROOT / "main.py"]
    names: set[str] = set()
