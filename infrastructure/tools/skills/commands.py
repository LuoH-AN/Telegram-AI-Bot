...
async def handle_skill_install(user_id: int, source: str, *, lang: str = "en") -> str:
    if not is_admin(user_id):
        return "❌ **技能安装仅限机器人管理员。**" if lang == "zh" else "❌ **Skill installation is restricted to bot administrators.**"
    source = source.strip()
    if not source:
