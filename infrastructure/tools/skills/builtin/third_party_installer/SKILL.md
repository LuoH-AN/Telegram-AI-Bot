...
version: 1.0.0
description: Guide third-party CLI setup and convert external integrations into this agent's runtime skills.
capabilities: [install_guidance, skill_generation, cli_verification]
platforms: [telegram]
---
...
4. If there is no native plugin for this Telegram AI Bot, create a prompt-only skill for this agent with `config_file`.
5. Write the skill to `runtime/plugins/<name>/SKILL.md` with frontmatter: name, version, description, repository, capabilities, platforms.
6. Put concrete usage rules in the body: which command to run, when to prefer it, how to verify it, and fallbacks.
7. Confirm the skill is registered by checking `/skill list` behavior when possible, or report that a restart is needed.
...
Generated prompt-only skills should be specific to the tool being installed, but the generation process is generic:

- Derive the skill name from the tool name, normalized for paths.
...
- Avoid hardcoding one vendor's installation flow into this skill; generate a tool-specific skill from the requested tool's docs each time.
