...
from .keypool import DEFAULT_SEARCH_DEPTH, KEY_POOL, TAVILY_ENDPOINT
...
def _normalize(raw: dict, top_k: int) -> list[dict]:
    ...
    for item in results[:top_k]:
        ...
        if len(snippet) > 300:
            snippet = snippet[:300] + "..."
        out.append({"title": str(item.get("title") or "").strip(), "url": str(item.get("url") or "").strip(), "snippet": snippet, "engine": "tavily", "score": item.get("score")})
    return out


def search_once(*, query: str, top_k: int, timeout_seconds: int) -> dict:
    if not KEY_POOL.snapshot()["configured"]:
    ...
    for _ in range(max(1, KEY_POOL.snapshot()["configured"])):
        ...
        try:
            resp = requests.post(
                TAVILY_ENDPOINT,
                json={"query": query, "max_results": top_k, "search_depth": DEFAULT_SEARCH_DEPTH},
        ...
        if resp.status_code >= 400:
            ...
            KEY_POOL.report_failure(key, kind, f"HTTP {resp.status_code} {preview}")
        ...
        return {"ok": True, "query": query, "count": len(raw.get("results") or []) if isinstance(raw, dict) else 0, "returned": len(normalized), "results": normalized, "answer": raw.get("answer") if isinstance(raw, dict) else None}
