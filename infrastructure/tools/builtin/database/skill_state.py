...
def _run(user_id: int, action: str, name: str, state: Any) -> ToolResult:
    ...
    if action == "set":
        ...
        if not state:
            ...
            commit()
            return ToolResult.text(f"Cleared state for skill '{name}'")
...
@tool(toolset="admin", side_effects=True, description="Read or overwrite a skill's persisted runtime state for the calling user. action: get|set. set takes a state object; set {} to clear.")
