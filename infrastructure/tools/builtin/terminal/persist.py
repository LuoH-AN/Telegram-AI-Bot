...
INSTALL_PATTERNS = (
    ...
    re.compile(r"(^|&&|\|\|)\s*python(3(?:\.\d+)?)?\s+-m\s+pip\s+install(\s|$)", re.IGNORECASE),
    re.compile(r"(^|&&|\|\|)\s*uv\s+pip\s+install(\s|$)", re.IGNORECASE),
    re.compile(r"(^|&&|\|\|)\s*uv\s+tool\s+install(\s|$)", re.IGNORECASE),
    re.compile(r"(^|&&|\|\|)\s*pipx\s+install(\s|$)", re.IGNORECASE),
    re.compile(r"(^|&&|\|\|)\s*npm\s+(i|install)(\s|$)", re.IGNORECASE),
    ...
    re.compile(r"(^|&&|\|\|)\s*composer\s+(require|install)(\s|$)", re.IGNORECASE),
    re.compile(r"(^|&&|\|\|)\s*poetry\s+(add|install)(\s|$)", re.IGNORECASE),
    re.compile(r"(^|&&|\|\|)\s*apt(-get)?\s+install(\s|$)", re.IGNORECASE),
    re.compile(r"(^|&&|\|\|)\s*(apk|dnf|yum)\s+(add|install)(\s|$)", re.IGNORECASE),
